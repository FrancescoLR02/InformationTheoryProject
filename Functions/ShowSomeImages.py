...
# ------------ resticted (round exactly -1 1) ------------------
# --------------------------------------------------------------

def ShowSomeImagesRestricted(model, testDataset, device, howmany=5):
   ...
   for i in range(howmany):
    ...
    with torch.no_grad():
         z, b, mean, logVar = model.Encoding(x, label)
         b_round = torch.sign(b)
         recon = model.Decoding(b_round)
